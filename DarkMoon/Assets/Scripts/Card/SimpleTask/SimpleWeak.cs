...
public class SimpleWeak : MonoBehaviour
{
    Field current_field = GameObject.Find("FieldManager").GetComponent<Field>();
    public void AddWeak(int entity_position_start, int entity_position_end, int amount)
    {
        ...
        {
            current_field.entity_list[i].entity_weak += amount;
        }
    }
}
